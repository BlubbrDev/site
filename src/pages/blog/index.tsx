        ...
        <section className="grid grid-rows-3 grid-cols-1 lg:grid-cols-2 lg:grid-flow-col gap-4 pt-4">
          <main className="row-span-3 col-span-1">
            ...
            <div>
              <Link href={"/blog/" + firstPost.id} passHref>
                <a>
                  <h2 className="font-serif text-3xl font-bold pt-4 cursor-pointer hover:text-blue transition duration-100 ease-in-out">
              ...
              index < 4 && (
                <div className="col-span-1 flex flex-row lg:pl-8">
                  ...
                  <div className="flex-1">
                    <Link href={"/blog/" + post.id} passHref>
                      <a>
                        <h2 className="font-serif text-xl font-semibold pb-2 cursor-pointer hover:text-blue transition duration-100 ease-in-out">
                          {post.data.title}
                        </h2>
                      </a>
                    </Link>
            ...
            {allPostsData.map((post) => (
              <div className="col-span-1 flex flex-row my-4">
                <Link href={"/blog/" + post.id} passHref>
                  <a>
                    <img
                      className="inline flex-none w-24 lg:w-48 lg:h-48 object-cover mr-4"
                      src={post.data.img}
                    />
                  </a>
                </Link>
                <div className="flex-1 w-100">
                  <Link href={"/blog/" + post.id} passHref>
                    <a>
                      <h2 className="font-serif text-lg lg:text-2xl font-bold pb-2 cursor-pointer hover:text-blue transition duration-100 ease-in-out">
                        {post.data.title}
                      </h2>
                    </a>
                  </Link>
                  <p className="font-sans text-md text-gray-700 py-3 truncate w-56 sm:w-auto sm:max-w-sm md:max-w-xs lg:max-w-sm xl:max-w-xl">
                    {firstPost.excerpt}
                  </p>
